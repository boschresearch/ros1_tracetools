 ...
 *
 * HINT: For user-defined tracing, you mostly need "ros::trace::message_processed".
 */
...
namespace ros {
	...
	namespace trace {
		...
		void timer_added(const void* fun_ptr,
		...
		*/
		void message_processed(const char* message_name,
		 ...
		 */
		void call_end(const void* ptr_ref, const void* data,
				const uint64_t trace_id);
		/** Trace queue delay experienced by the given function pointer */
		void queue_delay(const char* queue_name,
		 ...
		 */
		void subscriber_call_start(const std::string& topic,
		 ...
		 * */
		void subscriber_call_end(const std::string& topic,
						const void* queue_ref, const void* callback_ref,
						const void* message_ref,
						int receipt_time_sec, int receipt_time_nsec);
		 ...
		 */
		void timer_scheduled(const void* callback_ref,
		...
		/** Emit a trace message for a link in a processing chain.
				 ...
				 * */
		void link_step(const char* element_name,
